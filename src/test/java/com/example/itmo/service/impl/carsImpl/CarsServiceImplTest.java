...
public class CarsServiceImplTest {
    ...
    public void getAllCars() {
        ...
        when(mapper.convertValue(car1,  CarsInfoResponse.class)).thenReturn(carsInfoResponse1);
        when(mapper.convertValue(car2, CarsInfoResponse.class)).thenReturn(carsInfoResponse2);
    ...
    public void linkCarAndDriver() {
        ...
        when(mapper.convertValue(car, CarsInfoResponse.class)).thenReturn(carsInfoResponse);
    ...
    public void getUserCar() {
        ...
        UserInfoResponse userInfoResponse = new UserInfoResponse();
        when(userService.getUserDb(userId)).thenReturn(user);
        when(mapper.convertValue(any(Car.class), eq(CarsInfoResponse.class))).thenAnswer(invocation -> {
            Car car = invocation.getArgument(0);
            CarsInfoResponse response = new CarsInfoResponse();
        ...
        assertEquals(Optional.of(1L), actualCarsInfoResponses.get(0).getId());
        assertEquals(Optional.of(2L), actualCarsInfoResponses.get(1).getId());
